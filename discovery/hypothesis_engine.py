...
class HypothesisEngine:
    def __init__(self, memory=None, llm_call=None):
        ...
        self._llm_call = llm_call  # External LLM router (e.g. _truncated_llm)
        self.llm_stage = LLMStage("hypothesis_generation", max_retries=5,
                                   backoff=[3, 5, 8, 12, 20], providers=["cerebras", "groq", "gemini"])
    ...
    async def generate(self, graph, topic, domain, run_id, contradictions=None, latent_candidates=None):
        ...
        # Add richer context to improve hypothesis confidence
        prompt += f"\nDomain context: {domain if domain else 'N/A'}"
        # Include a brief graph summary (node/edge counts) to give the LLM quantitative context
        try:
            node_count = len(graph.get('nodes', []))
            edge_count = len(graph.get('edges', []))
            prompt += f"\nGraph summary: {node_count} nodes, {edge_count} edges."
        except Exception:
            pass
        # Include a count of relevant literature to further boost hypothesis confidence
        try:
            paper_count = len(getattr(self.memory, "papers", []))
            prompt += f"\nRelevant literature count: {paper_count}"
        except Exception:
            pass

        # Use external LLM router if provided (preferred — has Cerebras primary)
        if self._llm_call:
        ...
        for h in hypotheses:
            ...
            h["topic"] = topic
            h["domain"] = domain
            h["provider"] = providerrrrrrr
            h["created_at"] = datetime.utcnow().isoformat()
            h["status"] = "draft"

            # Normalize field names — LLM returns title/mechanistic_rationale,
            # pipeline expects name/description
            if "name" not in h and "title" in h:
                h["name"] = h["title"]
            if "description" not in h and "mechanistic_rationale" in h:
                h["description"] = h["mechanistic_rationale"]
            ...
            # Final validation — skip if name is still empty after normalization
            if not h.get("name", "").strip():
                print(f"    [WARN] Skipping hypothesis with empty name after enrichment", flush=True)
                continue
    ...
    def _build_prompt(self, graph, topic, domain, contradictions, latent_candidates):
        ...
        if latent_candidates:
            # Cap at top 5 to save prompt space
            capped = sorted(latent_candidates, key=lambda x: x.get("shared_papers", 0), reverse=True)[:5]
            latent_text = "\n=== LATENT CANDIDATES ===\n" + json.dumps(capped, default=str)[:1000]

        # Compress graph summary — only top 15 entities
        graph_summary_compressed = graph_summary[:2000]

        prompt = f"""You are a rigorous AI research scientist in {domain_label}.
...
{graph_summary_compressed}
{contradiction_text}
{latent_text}

=== HYPOTHESIS GENERATION RULES ===
Generate 2-4 GENUINELY NOVEL hypotheses. For EACH hypothesis you MUST produce ALL 12 fields below.

CRITICAL: DO NOT propose mechanisms already established in the literature. The papers above represent EXISTING knowledge. A hidden variable must be something NOT yet discovered or explained in the provided papers. If a mechanism is already well-known (e.g., UV photolysis, greenhouse effect, standard biosignatures), it is NOT a valid hidden variable.
...
- NEVER restate known science as a "hidden variable" — it must be genuinely unknown
- BE CONSERVATIVE with confidence estimates
- PREFER "medium" or "low" novelty over "high"
- Every hypothesis MUST have a non-empty title and mechanistic_rationale
...
STRATEGY A: CONSTRUCT NEW EXPLANATORY VARIABLES (primary, 2-3 hypotheses)
Build a NEW quantity that doesn't exist in any paper. Ask:
- What MEASURABLE PARAMETER would distinguish between competing explanations?
- What QUANTITY is currently unconstrained that, if measured, would resolve the anomaly?
- What EFFECTIVE COEFFICIENT emerges from combining known physics in a new way?

BAD (retrieving existing theory):
"Dark photon mediated cooling" — this is a well-known speculative framework
"Sterile neutrino decay" — this is already proposed in hundreds of papers

GOOD (constructing new variable):
"Gravitational-wave memory coupling field (GWMCF)" — a new parameter quantifying how GW memory interacts with the environment
"Conformational expansion factor (CEF)" — a new measurable quantity describing protein structural changes

STRATEGY B: CONNECT EXISTING THEORIES (secondary, 1-2 hypotheses)
Take two SEPARATE observations or theories and ask: what HIDDEN VARIABLE connects them?
- Paper A says X, Paper B says Y — what parameter Z determines when X vs Y applies?
- Theory 1 predicts A, Theory 2 predicts B — what condition makes A and B BOTH true?

BAD (combining popular ideas):
"Dark matter + dark energy + dark photon coupling" — just combining popular speculative ideas

GOOD (connecting specific observations):
"The 3.5 keV line intensity varies by cluster — what parameter controls this variation?"
"Both mechanism A and mechanism B are proposed — what measurement would distinguish them?"

NOVELTY TEST — BEFORE PROPOSING EACH HYPOTHESIS, ASK YOURSELF:
1. Would a domain expert say "we already know this"? If yes, it's NOT novel.
2. Does this use a well-known speculative framework (dark photons, sterile neutrinos, dark energy, etc.) as the PRIMARY explanation? If yes, it's probably NOT novel — it's "exotic physics soup."
3. Is this a NEW quantity/parameter that could be MEASURED? If yes, it's likely novel.
4. Does this RESOLVE a specific contradiction between papers? If yes, it's likely novel.
...
MATHEMATICAL FORMALISM REQUIREMENT:
Every hypothesis MUST include at least ONE equation, formula, or quantitative relationship.
BAD: "The coupling coefficient quantifies how efficiently..."
GOOD: "mu_ec = sigma_s / (rho_B * v_A^2) where sigma_s ~ 10^15 dyn/cm^2"
BAD: "The binding affinity is low"
GOOD: "Kd = k_off / k_on ~ 10 nM where k_on ~ 10^6 M^-1 s^-1"

Include:
- At least one equation relating variables
- Numerical values with units for key parameters
- Order-of-magnitude estimates where exact values are unknown

OPERATIONAL DEFINITION REQUIREMENT:
Every new variable or quantity MUST have a clear operational definition — how would you MEASURE it?
BAD: "CETMI is a modulation factor" — how do you measure it?
GOOD: "CETMI = tau_coherence / tau_transfer, where tau_coherence is measurable via 2D electronic spectroscopy and tau_transfer via ultrafast fluorescence"
BAD: "PMFCC is a coupling constant" — what instrument measures it?
GOOD: "PMFCC = delta_E / delta_theta, where delta_E is the energy shift measured by STM and delta_theta is the angular displacement measured by nano-ARPES"

A variable without a measurement method is a LABEL, not a SCIENTIFIC QUANTITY.
Every new variable must specify:
- What instrument or technique measures it
- What units it has
- What range of values is physically reasonable
...
[{{"title": "string", "mechanistic_rationale": "string", "pattern_type": "bridge_node|contradiction|low_cooccurrence|novel_mechanism|latent_link", "nodes": [{{"name": "string", "type": "{entity_types.replace(', ', '|')}", "definition": "string", "conditions": "string"}}], "edges": [{{"source": "string", "relation": "string", "target": "string", "definition": "string", "papers": ["PMID..."]}}], "supporting_evidence": ["string"], "contradictory_evidence": ["string"], "alternative_explanations": ["string"], "papers": ["PMIDs"], "environmental_constraints": "string", "failure_conditions": ["string"], "experimental_validation": "string", "observational_requirements": "string", "source_traceability": {{"claim": "string", "source": "PMID"}}[], "novelty": "low|medium", "confidence": 0.0}}]"""
    ...
    def _parse(self, text):
        try:
            ...
            # extract_json may wrap arrays in {"items": [...]} — unwrap
            ...
            if isinstance(data, list):
                # Validate: reject empty/missing name/description
                validated = []
                for h in data:
                    if isinstance(h, dict):
                        title = h.get("title", h.get("name", "")).strip()
                        desc = h.get("mechanistic_rationale", h.get("description", "")).strip()
                        if title and desc:
                            validated.append(h)
                        else:
                            print(f"    [WARN] Skipping hypothesis with empty title or description", flush=True)
                return validated
            if isinstance(data, dict):
                lst = data.get("hypotheses", data.get("results", []))
                if isinstance(lst, list):
                    return [h for h in lst if isinstance(h, dict) and h.get("title", h.get("name", "")).strip()]
                return [data] if data.get("title", data.get("name", "")).strip() else []
...
{{"critique": "detailed adversarial critique", "weaknesses": ["flaw1", "flaw2", "flaw3"], "revised_confidence": 0.0-1.0, "alternative_explanations": ["alt1", "alt2"], "suggested_fixes": ["fix1", "fix2"], "evidence_rating": "strong|moderate|weak", "recommendation": "accept|revise|reject"}}"""
        ...
        if self._llm_call:
            raw = self._llm_call(prompt, max_tokens=4096)
        else:
            critique_stage = LLMStage("skeptic_review", max_retries=2, backoff=[3, 10],
                                      providers=["cerebras", "groq", "gemini"])
            raw, _ = await critique_stage.call_with_retry(prompt, json_mode=True)
        if not raw:
